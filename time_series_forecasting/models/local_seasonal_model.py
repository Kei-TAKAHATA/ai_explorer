...
class LocalSeasonalModel:
    ...
    def create_state_transition_matrix(self, non_seasonal_dim: int, seasonal_periods: list[int], external_variable_dimension: int = 0):
        """
        状態遷移行列を作成
        ...
        seasonal_periods : list[int]
            季節成分の周期
        external_variable_dimension : int, optional
            外生変数の次元数
        ...
        state_dim = non_seasonal_dim + 2 * num_seasonal_components + external_variable_dimension  # バイアス成分 + 季節成分の合計 + 外生変数の数
    ...
    def fit(self, train_x, train_y, sigma2_e: float = None, sigma2_eta: float = None, sigma2_s: float = None):
        """観測データに基づいてモデルを学習する"""
        column_number = train_x.shape[1]
        # 外生変数の数（1次元は観測値なので除外）
        external_variable_dimension = column_number - 1
        # 状態誤差の分散
        ...
        state_dimension = non_seasonal_dim + 2 * number_of_seasonal_periods + external_variable_dimension
        ...
        self.F = self.create_state_transition_matrix(non_seasonal_dim, self.seasonal_periods, external_variable_dimension)  # 状態遷移行列
        # print("F の shape:", self.F.shape)  # 確認
        self.H = np.ones((observation_dimension, state_dimension))  # 観測行列 H の初期化
        # num_flags = train_x.shape[1] - (non_seasonal_dim - 1)  # フラグの数を計算
        # self.H = np.hstack((self.H, np.zeros((observation_dimension, external_variable_dimension))))  # フラグを考慮して観測行列 H を拡張
    ...
    def predict(self, input_vectors: np.ndarray = None, steps: int = 1, decay_factor: float = 0.9) -> np.ndarray:
        """
        入力ベクトルを用いて予測を行う。入力ベクトルがない場合は、ステップ数に基づいて予測を行う。

        Parameters
        ----------
        input_vectors : np.ndarray, optional
            予測に使用する入力ベクトル
        steps : int, optional
            予測するステップ数（入力ベクトルがない場合に使用）
        decay_factor : float, optional
            トレンド項に適用する減衰因子（入力ベクトルがない場合に使用）
        ...
        if input_vectors is None:
            # 入力ベクトルがない場合は、ステップ数に基づいて予測を行う
            for step in range(steps):
                next_state = self.F @ last_state_estimate
                if self.include_trend:
                    next_state[1] *= decay_factor ** step  # トレンド項に減衰因子を適用
                forecasted_states.append(next_state)
                last_state_estimate = next_state
        else:
            # 入力ベクトルがある場合は、それに基づいて予測を行う
            for input_vector in input_vectors:
                next_state = self.F @ last_state_estimate
                if self.include_trend:
                    next_state[1] *= decay_factor
                # 観測行列 H を入力ベクトルに基づいて調整
                # import pdb; pdb.set_trace()
                self.H[:, -input_vector.shape[0]:] = input_vector.reshape(1, -1)
                forecasted_states.append(next_state)
                last_state_estimate = next_state
    ...
    def predict_from_df(self, df: pd.DataFrame, observation_column_names: list[str] = None, predict_column_names: list[str] = None) -> pd.DataFrame:
        """
        ...
        # predict_values = self.predict_with_input(input_vectors=df.values)
        # import pdb; pdb.set_trace()
        predict_values = self.predict(input_vectors=df[observation_column_names].values)
        # import pdb; pdb.set_trace()
        # 予測結果を新しい列として追加
